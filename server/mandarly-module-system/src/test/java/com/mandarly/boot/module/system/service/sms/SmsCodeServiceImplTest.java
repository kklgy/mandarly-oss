...
import com.mandarly.boot.module.system.api.sms.dto.code.SmsCodeValidateReqDTO;
import com.mandarly.boot.module.system.dal.dataobject.sms.SmsCodeDO;
import com.mandarly.boot.module.system.dal.mysql.sms.SmsCodeMapper;
import com.mandarly.boot.module.system.enums.sms.SmsSceneEnum;
import com.mandarly.boot.module.system.framework.sms.config.SmsCodeProperties;
...
public class SmsCodeServiceImplTest extends BaseDbUnitTest {
    ...
    public void sendSmsCode_success() {
        ...
        // 断言调用
        verify(smsSendService).sendSingleSmsSync(eq(reqDTO.getMobile()), isNull(), isNull(),
                eq("user-sms-login"), eq(MapUtil.of("code", "9999")));
        verify(smsSendService, never()).sendSingleSms(any(), any(), any(), any(), any());
    }

    @Test
    public void sendSmsCode_sendFailureDeletesCode() {
        // 准备参数
        SmsCodeSendReqDTO reqDTO = randomPojo(SmsCodeSendReqDTO.class, o -> {
            o.setMobile("15601691300");
            o.setScene(SmsSceneEnum.MEMBER_LOGIN.getScene());
        });
        doThrow(new IllegalStateException("provider rejected")).when(smsSendService)
                .sendSingleSmsSync(eq(reqDTO.getMobile()), isNull(), isNull(),
                        eq("user-sms-login"), eq(MapUtil.of("code", "9999")));

        // 调用，并断言异常
        assertServiceException(() -> smsCodeService.sendSmsCode(reqDTO), SMS_CODE_SEND_FAILED);
        assertNull(smsCodeMapper.selectOne(null));
    }
