...
import com.mandarly.boot.framework.common.core.KeyValue;
...
import com.mandarly.boot.module.system.framework.sms.core.client.SmsClient;
import com.mandarly.boot.module.system.framework.sms.core.client.dto.SmsReceiveRespDTO;
import com.mandarly.boot.module.system.framework.sms.core.client.dto.SmsSendRespDTO;
...
import com.mandarly.boot.module.system.dal.dataobject.sms.SmsTemplateDO;
...
import com.mandarly.boot.module.system.mq.producer.sms.SmsProducer;
import com.mandarly.boot.module.system.service.member.MemberService;
...
public class SmsSendServiceImplTest extends BaseMockitoUnitTest {
    ...
    public void testSendSingleSmsSync_success() throws Throwable {
        // 准备参数
        String mobile = randomString();
        Long userId = randomLongId();
        Integer userType = randomEle(UserTypeEnum.values()).getValue();
        ...
        when(smsChannelService.getSmsClient(eq(template.getChannelId()))).thenReturn(smsClient);
        SmsSendRespDTO sendResult = randomPojo(SmsSendRespDTO.class, o -> o.setSuccess(true));
        when(smsClient.sendSms(eq(smsLogId), eq(mobile), eq(template.getApiTemplateId()),
                eq(Lists.newArrayList(new KeyValue<>("code", "1234"), new KeyValue<>("op", "login")))))
                .thenReturn(sendResult);

        // 调用
        Long resultSmsLogId = smsSendService.sendSingleSmsSync(mobile, userId, userType, templateCode, templateParams);

        // 断言
        assertEquals(smsLogId, resultSmsLogId);
        verify(smsProducer, never()).sendSmsSendMessage(anyLong(), anyString(), anyLong(), any(), anyList());
        verify(smsLogService).updateSmsSendResult(eq(smsLogId), eq(true),
                eq(sendResult.getApiCode()), eq(sendResult.getApiMsg()),
                eq(sendResult.getApiRequestId()), eq(sendResult.getSerialNo()));
    }

    @Test
    @SuppressWarnings("unchecked")
    public void testSendSingleSmsSync_apiFailureThrows() throws Throwable {
        // 准备参数
        String mobile = randomString();
        Long userId = randomLongId();
        Integer userType = randomEle(UserTypeEnum.values()).getValue();
        ...
        SmsClient smsClient = spy(SmsClient.class);
        when(smsChannelService.getSmsClient(eq(template.getChannelId()))).thenReturn(smsClient);
        SmsSendRespDTO sendResult = randomPojo(SmsSendRespDTO.class, o -> {
            o.setSuccess(false);
            o.setApiCode("FailedOperation");
            o.setApiMsg("send failed");
        });
        when(smsClient.sendSms(eq(smsLogId), eq(mobile), eq(template.getApiTemplateId()),
                eq(Lists.newArrayList(new KeyValue<>("code", "1234"), new KeyValue<>("op", "login")))))
                .thenReturn(sendResult);
        ...
        IllegalStateException exception = Assertions.assertThrows(IllegalStateException.class,
                () -> smsSendService.sendSingleSmsSync(mobile, userId, userType, templateCode, templateParams));
        assertEquals("短信发送失败: FailedOperation send failed", exception.getMessage());
        verify(smsLogService).updateSmsSendResult(eq(smsLogId), eq(false),
                eq("FailedOperation"), eq("send failed"),
                eq(sendResult.getApiRequestId()), eq(sendResult.getSerialNo()));
    }
    ...
    @Test
    public void testSendSingleSmsSync_errorWhenSmsTemplateDisable() {
        // 准备参数
        String mobile = randomString();
        Long userId = randomLongId();
        Integer userType = randomEle(UserTypeEnum.values()).getValue();
        ...
        IllegalStateException exception = Assertions.assertThrows(IllegalStateException.class,
                () -> smsSendService.sendSingleSmsSync(mobile, userId, userType, templateCode, templateParams));
        assertEquals("短信模板或渠道已禁用", exception.getMessage());
        verify(smsProducer, never()).sendSmsSendMessage(anyLong(), anyString(), anyLong(), any(), anyList());
    }
