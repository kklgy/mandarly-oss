...
import com.mandarly.boot.framework.test.core.util.RandomUtils;
...
import com.mandarly.boot.module.system.mq.producer.mail.MailProducer;
...
//                .setFrom("奥特曼 <ydym_test@163.com>")
    ...
    @Test
    public void testSendSingleMail_nullLogId_throwsBeforeMq() {
        String mail = randomEmail();
        Long userId = randomLongId();
        ...
        when(mailLogService.createMailLog(eq(userId), eq(userType),
                ...
                argThat(Collection::isEmpty),
                argThat(Collection::isEmpty),
                eq(account), eq(template), anyString(), eq(templateParams), eq(true))).thenReturn(null);

        assertThrows(IllegalStateException.class, () -> mailSendService.sendSingleMail(
                Lists.newArrayList(mail), null, null, userId, userType, templateCode, templateParams, (File[]) null));
        verify(mailProducer, never()).sendMailSendMessage(any(), any(), any(), any(), any(), any(), any(), any(), any());
    }
    ...
    public void testDoSendMail_exception() {
        try (MockedStatic<MailUtil> mailUtilMock = mockStatic(MailUtil.class)) {
            ...
            // 调用
            assertThrows(NullPointerException.class, () -> mailSendService.doSendMail(message));
            // 断言
    ...
    @Test
    public void testDoSendMail_nullLogId_throwsBeforeSend() {
        try (MockedStatic<MailUtil> mailUtilMock = mockStatic(MailUtil.class)) {
            MailSendMessage message = randomPojo(MailSendMessage.class, o -> o.setLogId(null));

            assertThrows(IllegalArgumentException.class, () -> mailSendService.doSendMail(message));
            mailUtilMock.verifyNoInteractions();
