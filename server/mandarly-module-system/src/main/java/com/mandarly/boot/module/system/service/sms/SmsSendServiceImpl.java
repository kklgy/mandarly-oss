...
public class SmsSendServiceImpl implements SmsSendService {
    ...
    public Long sendSingleSms(String mobile, Long userId, Integer userType,
        ...
        PreparedSms sms = prepareSingleSms(mobile, userId, userType, templateCode, templateParams);
        // 发送 MQ 消息，异步执行发送短信
        if (sms.isSend()) {
            smsProducer.sendSmsSendMessage(sms.sendLogId(), sms.mobile(), sms.channelId(),
                    sms.apiTemplateId(), sms.templateParams());
        }
        return sms.sendLogId();
    }

    @Override
    public Long sendSingleSmsSync(String mobile, Long userId, Integer userType,
                                  String templateCode, Map<String, Object> templateParams) {
        PreparedSms sms = prepareSingleSms(mobile, userId, userType, templateCode, templateParams);
        if (!sms.isSend()) {
            throw new IllegalStateException("短信模板或渠道已禁用");
        }
        doSendSms(sms.toMessage(), true);
        return sms.sendLogId();
    }

    private PreparedSms prepareSingleSms(String mobile, Long userId, Integer userType,
                                         String templateCode, Map<String, Object> templateParams) {
        // 校验短信模板是否合法
        ...
        if (sendLogId == null) {
            throw new IllegalStateException("短信发送日志编号不能为空");
        }

        return new PreparedSms(sendLogId, isSend, mobile, template.getChannelId(),
    ...
    public void doSendSms(SmsSendMessage message) {
        doSendSms(message, false);
    }

    private void doSendSms(SmsSendMessage message, boolean strict) {
        if (message.getLogId() == null) {
            throw new IllegalArgumentException("短信发送日志编号不能为空");
        }
        ...
        try {
            ...
            if (strict) {
                ...
                throw new IllegalStateException("短信发送失败", ex);
            }
            return;
        }
        ...
        if (strict && !Boolean.TRUE.equals(sendResponse.getSuccess())) {
            throw new IllegalStateException("短信发送失败: " + sendResponse.getApiCode() + " " + sendResponse.getApiMsg());
        }
    }
