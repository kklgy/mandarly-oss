...
public class MailSendServiceImpl implements MailSendService {
    ...
    public Long sendSingleMail(Collection<String> toMails, Collection<String> ccMails, Collection<String> bccMails,
                               ...
                               File... attachments) {
        PreparedMail mail = prepareSingleMail(toMails, ccMails, bccMails, userId, userType,
                templateCode, templateParams, attachments);
        // 发送 MQ 消息，异步执行发送邮件
        if (mail.isSend()) {
            mailProducer.sendMailSendMessage(mail.sendLogId(), mail.toMailSet(), mail.ccMailSet(), mail.bccMailSet(),
                    mail.accountId(), mail.nickname(), mail.title(), mail.content(), mail.attachments());
        }
        return mail.sendLogId();
    }

    @Override
    public Long sendSingleMailSync(Collection<String> toMails, Collection<String> ccMails, Collection<String> bccMails,
                                   Long userId, Integer userType,
                                   String templateCode, Map<String, Object> templateParams,
                                   File... attachments) {
        PreparedMail mail = prepareSingleMail(toMails, ccMails, bccMails, userId, userType,
                templateCode, templateParams, attachments);
        if (!mail.isSend()) {
            throw new IllegalStateException("邮件模板已禁用");
        }
        doSendMail(mail.toMessage());
        return mail.sendLogId();
    }

    private PreparedMail prepareSingleMail(Collection<String> toMails, Collection<String> ccMails, Collection<String> bccMails,
                                           Long userId, Integer userType,
                                           String templateCode, Map<String, Object> templateParams,
                                           File... attachments) {
        // 1.1 校验邮箱模版是否合法
        ...
        if (sendLogId == null) {
            throw new IllegalStateException("邮件发送日志编号不能为空");
        }
        return new PreparedMail(sendLogId, isSend, toMailSet, ccMailSet, bccMailSet,
    ...
    private record PreparedMail(
            Long sendLogId,
            Boolean isSend,
            Collection<String> toMailSet,
            Collection<String> ccMailSet,
            Collection<String> bccMailSet,
            Long accountId,
            String nickname,
            String title,
            String content,
        ...
        MailSendMessage toMessage() {
            return new MailSendMessage()
                    .setLogId(sendLogId)
                    .setToMails(toMailSet).setCcMails(ccMailSet).setBccMails(bccMailSet)
