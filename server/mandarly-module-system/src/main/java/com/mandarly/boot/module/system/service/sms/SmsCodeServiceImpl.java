...
import static com.mandarly.boot.framework.common.util.date.DateUtils.isToday;
...
public class SmsCodeServiceImpl implements SmsCodeService {
    ...
    public void sendSmsCode(SmsCodeSendReqDTO reqDTO) {
        ...
        // 创建验证码
        SmsCodeDO smsCode = createSmsCode(reqDTO.getMobile(), reqDTO.getScene(), reqDTO.getCreateIp());
        // 发送验证码
        try {
            smsSendService.sendSingleSmsSync(reqDTO.getMobile(), null, null,
                    sceneEnum.getTemplateCode(), MapUtil.of("code", smsCode.getCode()));
        } catch (Exception e) {
            if (smsCode.getId() != null) {
                smsCodeMapper.deleteById(smsCode.getId());
            } else {
                smsCodeMapper.deleteByMobile(reqDTO.getMobile(), smsCode.getCode(), reqDTO.getScene());
            }
            log.warn("[sendSmsCode][短信发送失败，mobile({}) scene({})]", reqDTO.getMobile(), reqDTO.getScene(), e);
            throw exception(SMS_CODE_SEND_FAILED);
        }
    }

    private SmsCodeDO createSmsCode(String mobile, Integer scene, String ip) {
        // 校验是否可以发送验证码，不用筛选场景
