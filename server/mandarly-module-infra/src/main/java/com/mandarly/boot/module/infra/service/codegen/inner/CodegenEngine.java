package com.mandarly.boot.module.infra.service.codegen.inner;
...
import com.mandarly.boot.framework.apilog.core.annotation.ApiAccessLog;
import com.mandarly.boot.framework.apilog.core.enums.OperateTypeEnum;
...
import com.mandarly.boot.framework.common.util.date.DateUtils;
import com.mandarly.boot.framework.common.util.date.LocalDateTimeUtils;
import com.mandarly.boot.framework.common.util.object.BeanUtils;
import com.mandarly.boot.framework.common.util.object.ObjectUtils;
import com.mandarly.boot.framework.common.util.string.StrUtils;
import com.mandarly.boot.framework.excel.core.annotations.DictFormat;
import com.mandarly.boot.framework.excel.core.convert.DictConvert;
import com.mandarly.boot.framework.excel.core.util.ExcelUtils;
...
import com.mandarly.boot.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.mandarly.boot.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.mandarly.boot.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.mandarly.boot.module.infra.enums.codegen.CodegenSceneEnum;
import com.mandarly.boot.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.mandarly.boot.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.mandarly.boot.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 mandarly-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 mandarly-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.mandarly.boot.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/mandarly-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/mandarly-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "mandarly-module-${table.moduleName}/" + // 顶级模块
                "mandarly-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "mandarly-module-${table.moduleName}/" + // 顶级模块
                "mandarly-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "mandarly-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "mandarly-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "mandarly-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "mandarly-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
